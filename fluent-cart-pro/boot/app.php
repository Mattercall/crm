        ...
        if ($licenseNotice) {
            ...
            add_filter('fluent_cart/admin_notices', function ($notices) use ($licenseNotice) {
                ...
                if (!empty($licenseNotice['message'])) {
                    $notices[] = [
                        'id'   => $licenseNotice['id'],
                        'type' => $licenseNotice['type'],
                        'html' => '<div>' . wp_kses_post($licenseNotice['message']) . '</div>'
                    ];
                }
            ...
            add_filter('fluent_cart/admin_notices', function ($notices) {
                if (FLUENTCART_MIN_CORE_VERSION !== FLUENTCART_VERSION && version_compare(FLUENTCART_MIN_CORE_VERSION, FLUENTCART_VERSION, '>')) {
                    ...
                    $notices[] = [
                        ...
                        'type' => 'core_update',
                        'html' => '<div>
                            FluentCart Base Plugin needs to be updated to the latest version.
                            <a href="' . esc_url($updateUrl) . '">Click here to update</a>
                        </div>'
                    ];
                }
                return $notices;
