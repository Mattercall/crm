...
class SPPLO_Stripe_Payment_Link_Orders {
  ...
  public static function init() {
    ...
    add_action('admin_footer', [__CLASS__, 'render_send_email_form']);
  ...
  public static function metabox_send_email($post) {
    ...
    $form_id = 'spplo_send_email_form';
    $nonce_field = wp_nonce_field('spplo_send_order_email', 'spplo_send_order_email_nonce', true, false);
    echo str_replace('<input ', '<input form="' . esc_attr($form_id) . '" ', $nonce_field);
    ...
    echo '<input type="text" class="widefat" id="spplo_custom_subject" name="spplo_custom_subject" value="' . esc_attr($default_subject) . '" form="' . esc_attr($form_id) . '" />';
    echo '</p>';
    echo '<p>';
    echo '<label for="spplo_custom_body"><strong>' . esc_html__('Body', 'spplo') . '</strong></label>';
    echo '<textarea class="widefat" rows="5" id="spplo_custom_body" name="spplo_custom_body" form="' . esc_attr($form_id) . '"></textarea>';
    echo '</p>';
    echo '<p>';
    echo '<label for="spplo_custom_download_link"><strong>' . esc_html__('Custom Download Link (optional)', 'spplo') . '</strong></label>';
    echo '<input type="url" class="widefat" id="spplo_custom_download_link" name="spplo_custom_download_link" placeholder="https://example.com/download" form="' . esc_attr($form_id) . '" />';
    echo '</p>';
    echo '<p>';
    echo '<button type="submit" name="spplo_send_action" value="custom" class="button button-primary" form="' . esc_attr($form_id) . '" formmethod="post" formaction="' . esc_url(admin_url('admin-post.php')) . '">';
  ...
  public static function render_send_email_form() {
    if (!is_admin()) {
      return;
    }

    $screen = function_exists('get_current_screen') ? get_current_screen() : null;
    if (!$screen || $screen->post_type !== self::CPT || $screen->base !== 'post') {
      return;
    }
